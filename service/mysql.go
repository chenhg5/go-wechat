...
func InitDB(username string, password string, port string, ip string, databaseName string) {
	...
	if err != nil {
		...
		SqlDB.SetMaxIdleConns(EnvConfig["DATABASE_MAX_IDLE_CON"].(int))   // 连接池连接数 = mysql最大连接数/2
		SqlDB.SetMaxOpenConns(EnvConfig["DATABASE_MAX_OPEN_CON"].(int))  // 最大打开连接 = mysql最大连接数
	}
}
